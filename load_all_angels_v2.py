...
from utils import check_data_exists  # опционально
...
def main():
    ...
    print(f"Найдено {len(txt_files)} файлов. Объединяем с маркерами...")

    all_text_parts = []
    for file_path in sorted(txt_files):  # сортируем для воспроизводимости
        file_name = os.path.basename(file_path)
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read().strip()
            if content:
                # Добавляем маркер начала документа
                all_text_parts.append(f"=== ДОКУМЕНТ: {file_name} ===\n{content}")
    ...
    # Объединяем с разделителем (двойной перенос строки между документами)
    raw_data = "\n\n".join(all_text_parts)
    print(f"Общий размер текста: {len(raw_data)} символов")

    # Извлекаем граф (функция уже разбивает текст на чанки)
    ...
    # Загружаем в Qdrant (весь текст с маркерами)
